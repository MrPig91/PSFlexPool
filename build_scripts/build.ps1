...
$moduleName = 'PSFlexPool'
$homedirectory = Join-Path -Path $env:GITHUB_WORKSPACE -ChildPath "PSFlexPool"
...
Add-Content -Path $modulePath -Value "Import-fpMinerAddress"
...
#$formatfiles = (Get-ChildItem -Path $formatsFolderPath).Name | ForEach-Object {".\Formats\$_"}
...
$manifestParameters = @{
     ...
     Author = "Mrpig91"
     Description = "A powershell module for interacting FlexPools API."
}
...
Publish-Module -Path $homedirectory -NuGetApiKey $env:PWSHGALLERY -Tags "FlexPool","API"
